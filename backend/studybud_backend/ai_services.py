...
def organize_notes(notes_corpus: str) -> Dict:
    """
    Automatically categorizes and structures notes using Together AI
    
    Args:
        notes_corpus (str): Raw text containing all notes to be organized
        
    Returns:
        Dict: Structured JSON containing categorized notes
    """
    try:
        response = requests.post(
            "https://api.together.xyz/v1/chat/completions",
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a note organization assistant. Analyze the provided notes and categorize them by topic, concept and importance. Only respond in JSON format."
                    },
                    {
                        "role": "user",
                        "content": f"Please organize these notes:\n{notes_corpus}"
                    }
                ],
                "response_format": {
                    "type": "json_schema",
                    "json_schema": {
                        "name": "notes_organization",
                        "strict": True,
                        "schema": NotesOrganization.model_json_schema()
                    }
                }
            }
        )
        print(f"[organize_notes] Raw API Response Status Code: {response.status_code}")
        print(f"[organize_notes] Raw API Response Text: {response.text}")
        ...
        try:
            ...
            print(f"[organize_notes] Attempting to parse cleaned content: {cleaned_content}")
            parsed_content = json.loads(cleaned_content)
            print(f"[organize_notes] Parsed Content: {parsed_content}")
            
            if not isinstance(parsed_content, dict):
                print(f"Invalid parsed content format: {parsed_content}")
                raise ValueError("API response content is not a valid object")
                
            # Validate required fields in response
            required_fields = ['schedule', 'priority_topics', 'estimated_time', 'resources']
            for field in required_fields:
                if field not in parsed_content:
        ...
        print(f"[organize_notes] API request failed: {str(e)}")
        raise ValueError(f"API request failed: {str(e)}")
    except Exception as e:
        print(f"[organize_notes] Unexpected error: {str(e)}")
        raise ValueError(f"Unexpected error while processing API response: {str(e)}")

def generate_study_plan(schedule_data: str) -> Dict:
    """
    Generates an optimized study plan using Together AI

    Args:
        schedule_data (str): JSON string containing schedule and task details

    Returns:
        Dict: Structured JSON containing the optimized study plan
    """
    try:
        response = requests.post(
            "https://api.together.xyz/v1/chat/completions",
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a study plan optimization assistant. Analyze the provided schedule and tasks, then generate an optimized study plan. Only respond in JSON format."
                    },
                    {
                        "role": "user",
                        "content": f"Please generate an optimized study plan based on this schedule and tasks:\n{schedule_data}"
                    }
                ],
                "response_format": {
                    "type": "json_schema",
        ...
        print(f"[generate_study_plan] Raw API Response Status Code: {response.status_code}")
        print(f"[generate_study_plan] Raw API Response Text: {response.text}")
        ...
        try:
            ...
            print(f"[generate_study_plan] Attempting to parse cleaned content")
            parsed_content = json.loads(cleaned_content)
            print(f"[generate_study_plan] Parsed Content: {parsed_content}")
        ...
        print(f"[generate_study_plan] API request failed: {str(e)}")
        raise ValueError(f"API request failed: {str(e)}")
    except Exception as e:
        print(f"[generate_study_plan] Unexpected error: {str(e)}")
        raise ValueError(f"Unexpected error while processing API response: {str(e)}")
...
class TimeSlot(BaseModel):
    ...
    activity: str = Field(description="Activity scheduled for this time slot")

class StudyTask(BaseModel):
    topic: str = Field(description="Topic to study")
    estimated_duration: int = Field(description="Estimated duration in minutes")
    priority: int = Field(description="Priority level from 1-5")
    prerequisites: List[str] = Field(description="List of topics that should be studied before this one", default=[])

class Schedule(BaseModel):
    daily_slots: List[TimeSlot] = Field(description="List of time slots with scheduled activities")
    study_tasks: List[StudyTask] = Field(description="List of study tasks to be scheduled")

class StudyPlan(BaseModel):
    scheduled_tasks: List[Dict[str, str]] = Field(description="List of scheduled study tasks with time slots")
    recommendations: List[str] = Field(description="Study recommendations and tips")
    estimated_completion_time: str = Field(description="Estimated total time to complete all tasks")

def generate_study_plan(schedule_data: str) -> Dict:
    """
    Generates an optimized study plan based on user's schedule and study tasks
    
    Args:
        schedule_data (str): JSON string containing daily schedule and study tasks
        
    Returns:
        Dict: Structured JSON containing the optimized study plan
    """
    if not schedule_data or not isinstance(schedule_data, str):
        raise ValueError("Invalid schedule_data: must be a non-empty JSON string")
        
    try:
        # Validate input is valid JSON
        json.loads(schedule_data)
    except json.JSONDecodeError as e:
        raise ValueError(f"Invalid JSON in schedule_data: {str(e)}")
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a study schedule optimizer. Analyze the provided schedule and tasks to create an efficient study plan. Consider time constraints, task priorities, and learning effectiveness. Only respond in JSON format."
                    },
                    {
                        "role": "user",
                        "content": f"Please create a study plan based on this schedule and tasks:\n{schedule_data}"
                    }
                ],
                "response_format": {
                    "type": "json_schema",
        ...
        try:
            ...
            print(f"Attempting to parse cleaned content")
            parsed_content = json.loads(cleaned_content)
